...
func makeApp() *cli.App {
	...
	app.Flags = []cli.Flag{
		...
		cli.IntFlag{
			...
			EnvVar: "NUM_WORKERS",
			Usage:  "The number of workers to use for crawling web-pages",
		},
		cli.DurationFlag{
			Name:   "update-interval",
			Value:  5 * time.Minute,
			EnvVar: "UPDATE_INTERVAL",
			Usage:  "The time between subsequent crawler runs",
		},
		cli.DurationFlag{
			Name:   "reindex-threshold",
			Value:  7 * 24 * time.Hour,
			EnvVar: "REINDEX_THRESHOLD",
...
func runMain(appCtx *cli.Context) error {
	...
	crawlerCfg.FetchWorkers = appCtx.Int("num-workers")
	crawlerCfg.UpdateInterval = appCtx.Duration("update-interval")
	crawlerCfg.ReIndexThreshold = appCtx.Duration("reindex-threshold")
	crawlerCfg.GraphAPI = graphAPI
	crawlerCfg.IndexAPI = indexerAPI
	crawlerCfg.PartitionDetector = partDet
	crawlerCfg.Logger = logger
	crawlSvc, err := crawler.NewService(crawlerCfg)
	if err != nil {
		return err
	}
	...
	go func() {
		defer wg.Done()
		if err := crawlSvc.Run(ctx); err != nil {
			logger.WithField("err", err).Error("crawler service exited with error")
			cancelFn()
		}
	}()
